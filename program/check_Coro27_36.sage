# check_Coro27_36.sage -- Corollary 27.36 (algebraic s=1) of d51coro.tex.
# f=(1+x)/(1-x^2)^{s/2}: a_{2k}=(2k)!/k! (s/2)_k, a_{2k+1}=(2k+1)a_{2k}.
#   HH_n = (prod_{k<n}(2k)!) prod_{j=1}^{n-1}(s+2j-2)^{n-j}.
# Run:  sage check_Coro27_36.sage N
import sys
...
_run("Corollary 27.36 (algebraic s=1): f=(1+x)/(1-x^2)^{s/2}",[("HH_n",lambda n:HH(n),base)])
