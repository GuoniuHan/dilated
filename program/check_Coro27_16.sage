# check_Coro27_16.sage -- Corollary 27.16 (cor:30) of d51coro.tex: Euler (s=3,t=0).
#   HH_n     = Omega(-3) 6^{-nbar} prod_i (2i)! prod_l (2l+3)! prod_m (2m)!
#   HH_n^(2) = (2n-1)!! 2^nbar (nbar+1)! (2nund-1)!! HH_n
# Run:  sage check_Coro27_16.sage N
import sys
...
def base(n):
    nb=(n+1)//2; nu=n//2
    return (Omega(-3,n)*6^(-nb)*prod(factorial(2*i) for i in range(n))
            *prod(factorial(2*l+3) for l in range(nb))*prod(factorial(2*m) for m in range(nu)))
def sh2(n):
    nb=(n+1)//2; nu=n//2
    return df(2*n-1)*2^nb*factorial(nb+1)*df(2*nu-1)*base(n)

_run("Corollary 27.16 (cor:30): Euler (s=3,t=0)",
