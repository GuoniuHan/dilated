# check_Coro27_33.sage -- Corollary 27.33 (elliptic m=1/2) of d51coro.tex.
# g_m = (1+sn(x,m))/cn(x,m);  HH_n(g_m) = (1-m)^C(n,2) HH_n(g_0),  HH_n(g_0)=2^{-C(n,2)} prod k!(2k)!.
# Run:  sage check_Coro27_33.sage N
import sys
...
xx=var('x')
def moments(Mmax):
    g=(1+jacobi('sn',xx,1/2))/jacobi('cn',xx,1/2)
...
def base(n): return ((1/4)^binomial(n,2))*prod(factorial(k)*factorial(2*k) for k in range(1,n))
_run("Corollary 27.33 (elliptic m=1/2): g_m=(1+sn)/cn",[("HH_n",lambda n:HH(n),base)])
