# check_Coro27_17.sage -- Corollary 27.17 (cor:xc0) of d51coro.tex: secant s=0, f=(1+x)/cos x.
# a_n = n![x^n] f.
#   HH_n     = 2^C(n,2) ((n-1)!!)^2 prod_{k=1}^{n-2} (k!!)^6
#   HH_n^(1) = det(a_{2i+j+1}) = ((n-1)!!)^2 HH_n
# Run:  sage check_Coro27_17.sage N
import sys
...
_A = secmoments(1, 3*N0+2)                      # s+1 = 1
def a(m): return _A[m]
def HH(n, sh=0): return matrix(QQ, n, n, lambda i, j: a(2*i+j+sh)).det()
def base(n): return 2^binomial(n,2)*df(n-1)^2*prod(df(k)^6 for k in range(1, n-1))
def sh1(n):  return df(n-1)^2*base(n)

_run("Corollary 27.17 (cor:xc0): secant s=0, f=(1+x)/cos x",
