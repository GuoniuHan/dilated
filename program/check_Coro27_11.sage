# check_Coro27_11.sage -- Corollary 27.11 (cor:23) of d51coro.tex: Euler (s=2,t=3).
#   HH_n     = 2^{-C(n+1,2)} 3^{-nund} prod_{k<n}(2k)! prod_{j=1}^{n+1} j!
#   HH_n^(1) = Lambda_n(2) (n(n+1)+1)          (t=3 carrier; Lambda_n(2) = eq:sHH23)
#   HH_n^(2) = (2n-1)!!(2nbar+1)!! 2^nund (nund+1)! HH_n
# Run:  sage check_Coro27_11.sage N
import sys
...
def base(n):
    nu=n//2
    return 2^(-binomial(n+1,2))*3^(-nu)*prod(factorial(2*k) for k in range(1,n))*prod(factorial(j) for j in range(1,n+2))
def Lam2(n):
    nb=(n+1)//2; nu=n//2
    return (2^(-binomial(n,2))/(2*nb+1) * 6^(-nb) * prod(factorial(2*l)*factorial(2*l+3) for l in range(nb))
            * 2^(-nu) * prod((2*c+3)*factorial(2*c+2) for c in range(nu))
            * prod(factorial(2*nb+2*r+1) for r in range(nu)))
def sh1(n): return Lam2(n)*(n*(n+1)+1)
def sh2(n):
    nb=(n+1)//2; nu=n//2
    return df(2*n-1)*df(2*nb+1)*2^nu*factorial(nu+1)*base(n)

_run("Corollary 27.11 (cor:23): Euler (s=2,t=3)",
