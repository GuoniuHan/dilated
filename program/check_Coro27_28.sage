# check_Coro27_28.sage -- Corollary 27.28 (cor:der1) of d51coro.tex: Springer derivative s=1.
# f = (cos x + sin x)/(cos x - sin x)^1;
#   HH_n = 4^C(n,2) (prod_{k<n} k!) prod_{j=0}^{n-2} [(2j+1)(2j+1+1)]^{n-1-j}.
# Run:  sage check_Coro27_28.sage N
import sys
...
def moments(Mmax):
    ...
    f=(cosx+sinx)*(cosx-sinx).inverse()^1
...
def base(n): return 4^binomial(n,2)*prod(factorial(k) for k in range(1,n))*prod(((2*j+1)*(2*j+1+1))^(n-1-j) for j in range(n-1))
_run("Corollary 27.28 (cor:der1): Springer derivative s=1",[("HH_n",lambda n:HH(n),base)])
