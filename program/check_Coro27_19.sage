# check_Coro27_19.sage -- Corollary 27.19 (cor:xc2) of d51coro.tex: secant s=2, f=(1+x)/cos^3 x.
#   HH_n     = c_n prod_{i=1}^{n-1}(i+2)!/2
#   HH_n^(1) = ((n-1)!!)^2 HH_n
# Run:  sage check_Coro27_19.sage N
import sys
...
def base(n): return cn(n)*prod(factorial(i+2)/2 for i in range(1, n))
def sh1(n):  return df(n-1)^2*base(n)

_run("Corollary 27.19 (cor:xc2): secant s=2, f=(1+x)/cos^3 x",
