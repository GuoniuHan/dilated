# check_Coro27_29.sage -- Corollary 27.29 (cor:der2) of d51coro.tex: Springer derivative s=2.
# f = (cos x + sin x)/(cos x - sin x)^2;
#   HH_n = 4^C(n,2) (prod_{k<n} k!) prod_{j=0}^{n-2} [(2j+2)(2j+2+1)]^{n-1-j}.
# Run:  sage check_Coro27_29.sage N
import sys
...
def moments(Mmax):
    ...
    f=(cosx+sinx)*(cosx-sinx).inverse()^2
...
def base(n): return 4^binomial(n,2)*prod(factorial(k) for k in range(1,n))*prod(((2*j+2)*(2*j+2+1))^(n-1-j) for j in range(n-1))
_run("Corollary 27.29 (cor:der2): Springer derivative s=2",[("HH_n",lambda n:HH(n),base)])
