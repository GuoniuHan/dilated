# check_Thm4_1.sage -- verification of Theorem 4.1 (thm:beta) of d02beta.tex.
#
# For the Beta family a_m = a0 * rho^m * (alpha)_m / (beta)_m and ANY strictly
# increasing integers 0 <= x_0 < x_1 < ... < x_{n-1},
#   det( a_{x_i+j} )_{0<=i,j<=n-1}
#     = rho^C(n,2) * prod_{i=0}^{n-1} (beta-alpha)_i
#         * prod_{0<=i<j<=n-1} (x_j - x_i)
#         * prod_{i=0}^{n-1} a_{x_i} / (x_i + beta)_{n-1}.
#
...
def RHS(n):
    ...
    return (rho^binomial(n, 2)
            * prod(poch(beta - alpha, i) for i in range(n))
            * vdm
            * prod(a(x[i]) / poch(x[i] + beta, n - 1) for i in range(n)))
...
if __name__ == "__main__":
    ...
    print("Theorem 4.1 (thm:beta) of d02beta.tex")
