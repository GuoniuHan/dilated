...
| Lemma 10.7 (atomic degeneration) | $`a_{2j}\big|_{s=-(2k+1)}=\sum_l\omega_l(-4l^2)^j`$; $`(s+2k+1)^{\underline n-k}\mid\ddot H_n^{(1)}`$ | [`check_Lem10_7.sage`](program/check_Lem10_7.sage) | [`.txt`](output/check_Lem10_7.txt) |
| Lemma 10.8 (rank of $`N`$) | $`\operatorname{rank}N\big|_{s=-(2k+1)}\le k+1`$; $`(s+2k+1)^{\underline n-1-k}\mid\det N`$ | [`check_Lem10_8.sage`](program/check_Lem10_8.sage) | [`.txt`](output/check_Lem10_8.txt) |
| Identity (10.8) (Chu–Vandermonde) | $`R=\sum_{j=0}^{k+1}\binom{k+X}{j}G_j(m)`$, and $`N_{r,m}\big|_{s=-(2k+1)}=(-1)^uR`$ | [`check_Eq10_8.sage`](program/check_Eq10_8.sage) | [`.txt`](output/check_Eq10_8.txt) |
